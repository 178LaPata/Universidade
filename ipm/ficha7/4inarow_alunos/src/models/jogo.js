...
export class Jogo {
  ...
  validarSeTerminou() {
    for (let index = 0; index < this.tabuleiro.length; index++) {
      ...
      for (let index2 = 0; index2 < coluna.length; index2++) {
        ...
        // Construir a string para a diagonal inferior que começa na posição (index,index2) 
        let diagonalInferior = this.getCoordenadasDiagonalInferior(index, index2)
                                  .map((x) => this.getCelulaInterna(x[0],x[1]))
                                  .join("");
        // Ver se alguém venceu na diagonal inferior.
        this.vencedor = diagonalInferior.includes("falsefalsefalsefalse") ? ResultadoJogo.AMARELO :
                        (diagonalInferior.includes("truetruetruetrue") ? ResultadoJogo.VERMELHO : null);
        // Se houver um vencedor, o jogo terminou.
        if (this.vencedor !== null) {
          this.terminado = true;
          return;
  ...
  */
  tabuleiroCheio() {
    // TODO: Implementar a função tabuleiroCheio.
    for (let index = 0; index < this.tabuleiro.length; index++) {
      const coluna = this.tabuleiro[index];
      for (let index2 = 0; index2 < coluna.length; index2++) {
        let coluna = this.getCoordenadasColuna(index, index2)
  ...
  O tabuleiro fica vazio, o jogo não terminou, o jogador a jogar é escolhido 
  aleatoriamente e o vencedor é ResultadoJogo.EMPATE (valor irrelevante por agora).
  */
  reset() {
    // TODO: Implementar a função reset.
    this.tabuleiro = [];
    for (let index = 0; index < Jogo.NUMERO_COLUNAS; index++) {
      this.tabuleiro.push([]);
    }
    this.jogadorAJogar = Math.random() < 0.5;
    this.vencedor = ResultadoJogo.EMPATE;
